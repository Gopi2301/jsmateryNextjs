...
const ResourceCard = ({ id, title, image, downloadNumber }: Props) => {
  return (
    <div>
      <Card className="w-full max-w-fit border-0 !bg-tranparent sm:max-w-[256px]">
        
        <Link href={`/resource/${id}`}>
        <CardHeader className="flex-center flex-col gap-2.5 !p-0">
            <div className="h-full rounded-md object-cover">
        ...
        <div className="flex-center body-medium gap-1.5 text-white">
            <Image className="" src='/downloads.svg' width={20} height={20} alt='download'/>
        {`${downloadNumber}`}
        </div>
